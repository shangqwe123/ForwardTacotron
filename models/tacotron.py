...
class Tacotron(nn.Module):
    ...
    def forward(self, x, m, sp_id, generate_gta=False):
        device = next(self.parameters()).device  # use same device as parameters
    ...
    def generate(self, x, sp_id, steps=2000):
        self.eval()
        device = next(self.parameters()).device  # use same device as parameters

        batch_size = 1
        x = torch.as_tensor(x, dtype=torch.long, device=device).unsqueeze(0)
        sp_id = torch.as_tensor(sp_id, dtype=torch.long, device=device).unsqueeze(0)
