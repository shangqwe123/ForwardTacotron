...
if __name__ == '__main__':
    ...
    for i, x in enumerate(inputs, 1):
        ...
        _, m, m2, _ = tts_model.generate(x, alpha=args.alpha)

        # Fix mel spectrogram scaling to be from 0 to 1
        m = (m + 4) / 8
        m2 = (m2 + 4) / 8
        np.clip(m, 0, 1, out=m)
        np.clip(m2, 0, 1, out=m2)
        ...
        if input_text:
            ...
            save_path2 = paths.forward_output/f'{i}_{v_type}_{tts_k}ko2.wav'

        if args.vocoder == 'wavernn':
            m = torch.tensor(m).unsqueeze(0)
            m2 = torch.tensor(m2).unsqueeze(0)
            wav =voc_model.generate(m, save_path, batched, hp.voc_target, hp.voc_overlap, hp.mu_law)
            wav2 =voc_model.generate(m2, save_path2, batched, hp.voc_target, hp.voc_overlap, hp.mu_law)
            wavs.append(wav)
